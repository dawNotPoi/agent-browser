...
mod tests {
    ...
    #[tokio::test]
    async fn seeded_frame_populates_static_recording_and_contact_sheet() {
        let image = image::RgbImage::from_pixel(64, 64, image::Rgb([12, 34, 56]));
        let mut png = std::io::Cursor::new(Vec::new());
        image.write_to(&mut png, image::ImageFormat::Png).unwrap();
        let image_data = Arc::new(png.into_inner());
        let (frame_tx, mut frame_rx) = mpsc::channel(1);
        let (contact_tx, contact_rx) = std::sync::mpsc::sync_channel(1);

        seed_recording_outputs(
            &frame_tx,
            Some(&contact_tx),
            CapturedVideoFrame {
        ...
        let path = dir.path().join("static.contact-sheet.png");
        write_contact_sheet(&path, &frames).unwrap();
        assert!(path.is_file());
    }
    ...
    fn test_contact_sheet_compacts_high_rate_scroll_burst() {
        ...
        for index in 0..=60u32 {
            ...
            image::DynamicImage::ImageRgb8(source)
                .write_to(&mut std::io::Cursor::new(&mut png), image::ImageFormat::Png)
                .unwrap();
            collector.consider(&png, Duration::from_millis(index as u64 * 16));
            last_png = png;
        }
        // A held frame ends the burst and must still become the final cell.
        collector.consider(&last_png, Duration::from_millis(1_300));
