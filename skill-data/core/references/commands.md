...
Browser Use Cloud lifecycle: setup is bounded to 18s plus up to 4s for timeout cleanup (10s create, 8s CDP attach, 4s stop). `close` succeeds only after the Cloud session acknowledges it stopped; a failed stop returns an error, keeps the session id for retrying `close`, and blocks a new launch until released. If create fails before an id arrives, the outcome is unknown; inspect the Browser Use Cloud dashboard before retrying. Each session writes a recovery receipt at `<socket dir>/browser-use-<id>.receipt` (contains the browser id and manual stop steps, never the API key); removal after an acknowledged stop is best effort, so verify the status in Cloud before acting on a leftover receipt.
