...
Chrome recordings do not normally include the native pointer. Add `--cursor` for an animated pointer and click ripple. During a drag, the pointer follows the captured page frame. Releasing the mouse button resumes free pointer movement immediately, even when the page does not repaint:
